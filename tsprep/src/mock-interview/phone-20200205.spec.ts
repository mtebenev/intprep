...
/**
 * Out of memory (keep the raw data)
 */
class SnapshotArrayBad {

  private snapshots: number[][];
  private currentData: number[];
  private readonly length: number;
  private readonly rowFact: () => number[];

  constructor(length: number) {
    this.length = length;
    this.snapshots = [];
    this.rowFact = () => Array.from(Array(this.length).keys()).map(k => 0);
    this.currentData = this.rowFact();
  }

  public set(index: number, val: number): void {
    if(index <= this.length) {
      this.currentData[index] = val;
    }
  }

  public snap(): number {
    this.snapshots.push([...this.currentData]);
    return this.snapshots.length - 1;
  }

  public get(index: number, snapId: number): number {
    return snapId < this.snapshots.length && index < this.length
      ? this.snapshots[snapId][index]
      : 0;
  }
}
